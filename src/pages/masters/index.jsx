...
const MasterListPage = ({ userName, adminsList = [], setAdminsList }) => {
  ...
  const onClickRefresh = async () => {
    ...
    try {
      ...
      toast.success("Updated Masters list !!!", {
        autoClose: 1000,
      });
    } catch (e) {
  ...
  const usersList =
    !isEmpty(adminsList) && adminsList[0]
      ? adminsList.filter((item) => item.userRole === "master")
      : [];
  return (
    <Fragment>
      {isAddUser && (
        <AddAdmin
          ...
          newRole="master"
          successMsg="Master Created Successful"
        />
      )}
      ...
      <UsersStyle.Wrapper dullOpacity={isAddUser || isConsent}>
        ...
        <AccountsPage
          ...
          isLoading={isLoading}
          usersList={usersList}
          addBtnTitle={"Add Master"}
          tableColHeadings={MasterTableColHeadings}
          navigateToDetails={RouteConfig.masterDetails}
          type="masters"
        />
      </UsersStyle.Wrapper>
    </Fragment>
...
export default connect(mapStateToProps, mapDispatchToProps)(MasterListPage);
